...
def mean_std(data):    
    ...
    # steps = data[0].ground_truth.shape[0]
    ...
    for geom in range(len(data)):        
        ...
        # node_mean = node_mean + torch.mean(nd_f, dim = 1, keepdim = True)
...
def load_graph(path=None, data_types=None, steps=101, split =None ,shuffle = None):
    ...
    for geomind in range(num_files):
        ...
        if field1.shape[0] != steps:
            continue        
        
        print(f"geometry {geomind}")
        ...
        field = torch.cat([field1, field2], dim=-1)      

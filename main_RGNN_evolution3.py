...
import model_RGNN_evolution3 as model
from utils_RGNN_evolution3 import*
...
# Create DataLoader with a window of sequences #ps I have added this
def create_graph_sequences(dataset, window):
    sequences = []
    for i in range(len(dataset) - window + 1):
        sequence = [dataset[j] for j in range(i, i + window)]
...
graph_sequences = create_graph_sequences(dataset, learned_model._window) #ps I have added it
dataloader = DataLoader(graph_sequences, batch_size=1, shuffle=True) #ps I have added it
#dataloader = DataLoader(dataset, batch_size = 1, shuffle = True) #ps previous one
import time

def train(model, dataloader): #ps I have added it
    model.train()           
    loss_all = 0 
    ii=0
    for batch in dataloader:
        ii=ii+1
        print(f"Epoch {epoch}, Batch size: {len(dataloader)}")
        print(f"Batch no {ii}")
        
        # Move the entire DataBatch to the device
        batch = [graph.to(device) for graph in batch]  # Since batch is a list, we take the first element and move it to the device
        
        # Process the graph batch with the model
        out = model(batch)  # The batch is already moved to the device, no need to move individual graphs
        
        # Extract ground truth
        ground_truth = torch.stack([graph.y for graph in batch]).to(device)  # Assuming the labels are stored in batch.y
        
        # Compute loss
          # Remove the extra dimension, so out becomes (N, 6) out = out.squeeze(0)
...
def test_f(model, test_data, geom):   
    ...
    for step in range(start_step,steps,delta_step):      
        ...
        start_time = time.time() 
        #out = model(graph)
        out = model([graph])  # Wrap the single graph into a list to make it a sequence
...
print(f"My initial shape is {field.shape}")
